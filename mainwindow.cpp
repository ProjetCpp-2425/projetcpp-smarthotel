    ...
    ui(new Ui::MainWindow)
{
   ...
   // highlightDatesFromTableView(); // Applique les couleurs aux dates dès le lancement
    ...
    ui->label_stat->setPixmap(pixstat.scaled(300,300,Qt::KeepAspectRatio));
    QPixmap pixstate("C:/Users/user/Downloads/stat.png");
    ui->label_state->setPixmap(pixstate.scaled(150,150,Qt::KeepAspectRatio));
    QPixmap pixlogo("C:/Users/user/Downloads/logo.png");
    ...
    ui->label_logoo->setPixmap(pixlogoo.scaled(100,100,Qt::KeepAspectRatio));
    QPixmap pixlog("C:/Users/user/Downloads/logo.png");
    ui->label_log->setPixmap(pixlog.scaled(100,100,Qt::KeepAspectRatio));
    QPixmap pixcal("C:/Users/user/Downloads/calee-removebg-preview.png");
    ui->label_cal->setPixmap(pixcal.scaled(300,300,Qt::KeepAspectRatio));
    QPixmap pixqrc1("C:/Users/user/Downloads/qrcode.png");
    ui->label_qrc1->setPixmap(pixqrc1.scaled(150,150,Qt::KeepAspectRatio));
...
{    bool ok = false;
    ...
    QString text = QString("ID_MAINTENANCE: %1\nID_EMPLOYE: %2\nNUM_CHAMBRE: %3\nTYPE_MAINTENANCE: %4\nDATE_DEBUT: %5\nDATE_FIN: %6\nETAT_MAINTENANCE: %7\nPRIORITE: %8")
                       .arg(ID_MAINTENANCE)
                       .arg(ID_EMPLOYE)
                       .arg(NUM_CHAMBRE_CONCERNEE)
                       .arg(TYPE_MAINTENANCE)
                       .arg(DATE_DEBUT.toString("yyyy-MM-dd"))
                       .arg(DATE_FIN.toString("yyyy-MM-dd"))
                       .arg(ETAT_MAINTENANCE)
                       .arg(PRIORITE);
...
void MainWindow::afficherMaintenancesPourDate(const QDate &date) {
    ...
    qDebug() << "Date sélectionnée : " << date.toString("yyyy-MM-dd");
    ...
    query.bindValue(":selectedDate", date.toString("yyyy-MM-dd"));

    if (!query.exec()) {
        QMessageBox::warning(this, "Erreur", "Impossible de récupérer les maintenances : " + query.lastError().text());
        return;
    }
    ...
    while (query.next()) {
        ...
        QString numChambre = query.value("NUM_CHAMBRE_CONCERNEE").toString();
        QString typeMaintenance = query.value("TYPE_MAINTENANCE").toString();
        QString dateDebut = query.value("DATE_DEBUT").toDate().toString("yyyy-MM-dd");
        QString dateFin = query.value("DATE_FIN").toDate().toString("yyyy-MM-dd");
        QString etatMaintenance = query.value("ETAT_MAINTENANCE").toString();
        QString priorite = query.value("PRIORITE").toString();

        // Ajouter une ligne dans le tableau
        ui->commandesTableWidget->insertRow(row);
        ui->commandesTableWidget->setItem(row, 0, new QTableWidgetItem(idMaintenance));
        ui->commandesTableWidget->setItem(row, 1, new QTableWidgetItem(idEmploye));
        ui->commandesTableWidget->setItem(row, 2, new QTableWidgetItem(numChambre));
        ui->commandesTableWidget->setItem(row, 3, new QTableWidgetItem(typeMaintenance));
        ui->commandesTableWidget->setItem(row, 4, new QTableWidgetItem(dateDebut));
        ui->commandesTableWidget->setItem(row, 5, new QTableWidgetItem(dateFin));
        ui->commandesTableWidget->setItem(row, 6, new QTableWidgetItem(etatMaintenance));
        ui->commandesTableWidget->setItem(row, 7, new QTableWidgetItem(priorite));
